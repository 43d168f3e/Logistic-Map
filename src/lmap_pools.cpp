...
//public:
    ...
    data_block(tup_idANDbeginANDendANDarea const &ibeL)
      : id_{0}, begin_{0}, end_{0}, area_{0}
    {
...
if ( m_id == (nthreads - 1) ) {
    ...
    std::memcpy(dest, src, nrows * screenWidth * sizeof(vectorRGB::value_type)  );// * screenWidth * sizeof(vectorRGB::value_type));
            ...
            Q_RGB_t* buffer = bbuffer.get();//new Q_RGB_t[screenHeight * screenWidth];
...
quotientANDmodulo_t quotientANDmodulo(Q_RGB_t numTHREADS, Q_RGB_t screenHeight) {
    quotient_t quotient    = screenHeight / numTHREADS;//static_cast<std::uint32_t> ( screenHeight/numTHREADS );
    modulo_t modulo      = screenHeight % numTHREADS;
    assert(screenHeight == quotient * numTHREADS + modulo);
    Q_ASSERT(screenHeight == quotient * numTHREADS + modulo);
...
vec_idANDbeginANDendANDarea calculate_area_begin_end_id(Q_RGB_t numTHREADS, Q_RGB_t screenWidth, Q_RGB_t screenHeight) {
    vec_idANDbeginANDendANDarea vbeL;
    vbeL.reserve(numTHREADS);
    begin_t begin{0};
    end_t end{0};
    area_t area{0};
    auto [nrows, remainder] = quotientANDmodulo(numTHREADS, screenHeight);

auto indices = ranges::view::iota(0u, numTHREADS);
//PLN: for (const auto& b : indices) is replaced by
     RANGES_FOR(auto i, indices) {
                const auto& b=i;
    }
    ...
    if (id == (numTHREADS - 1)) {
        end    = static_cast<end_t>(screenHeight - 1);
        area = static_cast<area_t>( (nrows + remainder) * screenWidth );
        //memset(_d.get(), 0, (nrows + remainder) * screenWidth * sizeof (Q_RGB_t));
    }
    else {
        end    = static_cast<end_t>( (id+1)*nrows - 1);
        area = static_cast<area_t>( nrows * screenWidth );
        //memset(_d.get(), 0, nrows * screenWidth * sizeof (Q_RGB_t));
    }
        vbeL.emplace_back(static_cast<ID_t>(id), begin, end, area);
   }
#if 0
