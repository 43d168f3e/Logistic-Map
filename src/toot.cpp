...
#else /* UNIX */
#if 0
If you are working on Linux with GCC, then by default it works in -std=gnu11 or -std=gnu90 mode,
and the POSIX (and some other) sets of symbols are made visible
without you needing to set _XOPEN_SOURCE or _POSIX_C_SOURCE or other similar macros.
If you use -std=c11 or -std=c99 or (perish the thought) -std=c90,
then POSIX and GNU symbols are not available by default;
you have to define _XOPEN_SOURCE or _POSIX_C_SOURCE to see the POSIX symbols.
...
static char toot_version[8] = "1.0.6"; /* contains tinyfd current version number */

static int toot_verbose = 0; /* 0 (default) or 1 : prints the command line calls */
...
void toot(float aFrequency_Hz, int aLength_ms, SignalHandler func_callback=nullptr, OGLWidget * ptr=nullptr)
{
#ifndef _WIN32
...
#endif /* UNIX */
    if (func_callback != nullptr) (*func_callback)(ptr);
}
///////////////////////////////////////////////////////////////////////////////
void soot(float aFrequency_Hz, int aLength_ms)
{
#ifndef _WIN32
    char lDialogString[256];
    FILE * lIn;
